...
const RULES: Language[] = [
  ...
  rule("adversary.typescript.build.output", "packaging", "medium", "Build output is not publish-ready", "Build output is not publish-ready", "The project lacks a build command, declared entrypoint, or current build artifact required for packaging.", "The runtime can only execute the JavaScript artifact named by the manifest after the package build completes.", "Packaging fails or ships stale code that differs from the reviewed TypeScript source.", "Provide a deterministic build script and verify it produces the declared entrypoint; rebuild committed dist output after source changes.", "small"),
