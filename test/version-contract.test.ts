...
test("manifest, package, lock, runtime, bundle, and artifact assertion share one version", async () => {
  const [manifestText, packageText, lockText, source, bundle, artifactTest, ignore] = await Promise.all([
    ...
    readFile(new URL("../.adversaryignore", import.meta.url), "utf8"),
  ]);
  const version = parseAdversaryManifest(manifestText).version;
  const packageJson = JSON.parse(packageText);
  const lock = JSON.parse(lockText);
  assert.equal(version, "0.0.30");
