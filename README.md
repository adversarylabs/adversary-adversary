...
Source text is split into bounded, host-prepared citations. The model selects a citation ID and a line inside its declared range, so grounding does not depend on a provider reproducing source text exactly. An invalid citation gets one repair attempt; if it still cannot be verified, model observations are omitted and the deterministic review completes with a visible note instead of failing the run.
